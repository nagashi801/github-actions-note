...
const system = [
  ...
  'Every level-2 and level-3 section needs one visual article illustration. Include one vivid imagePrompt for every section, including small headings. These are decorative mood images that improve the visual rhythm of the article, not proof of actual production work.',
  ...
  'Image prompts must be in English, 16:9, polished editorial illustration or cinematic photo style. They should match the mood of each section, not explain the section with text.',
  ...
  'Hands-on demo articles must feel like a real build log: Step1 input, actual or placeholder screenshot/result, what was wrong, exact fix, then next step. Do not write abstract tutorial paragraphs for more than 3 sentences without a concrete example.',
  ...
  'Do not create demoAssets. The automation only creates one decorative illustration for each heading. Actual production screenshots, Gemini scene images, Kling videos, and CapCut screenshots are inserted manually by the user.',
  ...
  'When showing model output, separate the prompt block and output block into different paragraphs. Use a standalone label like "---出力結果---" followed by a blank line, then the output text. Do not write "---出力開始---はい、承知いたしました！" on the same line.',
