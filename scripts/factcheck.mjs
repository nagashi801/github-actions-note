...
const system = [
  ...
  'Every level-2 and level-3 section needs one visual article illustration. Keep or update one imagePrompt for every section. These are decorative mood images that improve the visual rhythm of the article, not proof of actual production work.',
  'Keep each sentence as its own paragraph with a blank line after every sentence.',
  'Do not add demoAssets or [[demo_image:...]] markers. The automation only creates one decorative illustration for each heading.',
  ...
  'When showing model output, separate the prompt block and output block into different paragraphs. Use a standalone label like "---出力結果---" followed by a blank line, then the output text.',
